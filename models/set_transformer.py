...
class RadiomicsSetTransformer(pl.LightningModule):
    def __init__(self, input_dim: int, config: dict):
        super(RadiomicsSetTransformer, self).__init__()
        self.save_hyperparameters()
        self.model = SetTransformer(dim_input=input_dim, num_outputs=1, dim_output=config['num_classes'])
...
if __name__ == '__main__':
    model = SetTransformer(dim_input=10, num_outputs=1, dim_output=3)
    x = torch.randn([7,20,10])
    out = model(x)
    print(out.shape)